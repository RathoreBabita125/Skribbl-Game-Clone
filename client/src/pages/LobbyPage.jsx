          ...
          {roomSettings && (
            <div className="settings-summary card">
              ...
              <div className="settings-pills">
                <span className="pill">👥 {roomSettings.maxPlayers} players</span>
                <span className="pill">🔄 {roomSettings.rounds} rounds</span>
                <span className="pill">⏱️ {roomSettings.drawTime}s</span>
                <span className="pill">💡 {roomSettings.hints === 0 ? 'No hints' : `${roomSettings.hints} hints`}</span>
                <span className="pill">📝 {roomSettings.wordCount} words</span>
              </div>
            </div>
          )}
            ...
            <div className="players-grid">
              {players.map((player, i) => (
                <div key={player.id} className={`player-card ${player.id === myPlayer?.id ? 'me' : ''}`} style={{animationDelay:`${i * 0.05}s`}}>
                  <div className="player-avatar" style={{background: `hsl(${(player.avatar || 1) * 30}, 70%, 50%)`}}>
                    {AVATAR_EMOJIS[(player.avatar || 1) % AVATAR_EMOJIS.length]}
                  </div>
